...
class GameViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        baseNode.position = SCNVector3(0, 0, -1)
        scene.rootNode.addChildNode(baseNode)
        
        let sunNode = SCNNode()
        let sun = SCNSphere(radius: 0.125)
        ...
        earthGroupNode.position = SCNVector3(0.75, 0, 0)
        earthRotationNode.addChildNode(earthGroupNode)
        
        let earthNode = SCNNode()
        let earth = SCNSphere(radius: 0.075)
        ...
        let sunHalo = SCNPlane(width: 1.5, height: 1.5)
